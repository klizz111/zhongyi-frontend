...
import { TextInput, Select, Textarea, Button, Paper, Stack, Title, Text, Group, Box, Loader } from '@mantine/core';
...
${data.allergies ? `过敏史：${data.allergies}` : ''}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 验证必填字段
    if (!formData.name || !formData.age || !formData.gender || !formData.maritalStatus || !formData.condition) {
      setError('请填写所有必填项');
      setLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch('http://hk.klizz.asia:7777/api/translation', {
        method: 'POST',
        headers: {
      ...
      if (data.response) {
        setSuccess('信息提交成功！');
        setResponse(data.response); // 使用 response 字段而不是 text
      } else {
        ...
        <form onSubmit={handleSubmit}>
          <Stack spacing={20}>
            ...
            <Group position="apart">
              ...
              <Button variant="outline" color="gray" onClick={clearForm} disabled={loading}>
                清空表单
              </Button>
            </Group>
          </Stack>
